		...
		<ul>
			<li>
				...
				<c:if test="${articlePage.startPage > 5}">
					<a href="Search_Category_Commerce.do?category=digit&pageNo=${articlePage.startPage - 5}">[이전]</a>
				</c:if> 
				<!-- startPage to endPage --> 
				<c:forEach var="pNo" begin="${articlePage.startPage}" end="${articlePage.endPage}">
					<a href="Search_Category_Commerce.do?category=digit&pageNo=${pNo}">${pNo}</a>
				</c:forEach> 
				<!-- endPage가 총페이지보다 작을 시에 다음 링크 -->
				<c:if test="${articlePage.endPage < articlePage.totalPages}">
					<a href="Search_Category_Commerce.do?category=digit&pageNo=${articlePage.startPage + 5 }">[다음]</a>
				</c:if>
			</li>
		</ul>
	</c:if>
